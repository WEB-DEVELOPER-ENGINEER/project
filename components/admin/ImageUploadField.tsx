...
interface ImageUploadFieldProps extends Omit<InputProps, 'source'> {
  source: string
  label?: string
  accept?: string
  helperText?: string
...
export const ImageUploadField: React.FC<ImageUploadFieldProps> = ({
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    // Ensure we return a clean URL string
    const url = result.data.url.toString()
  ...
  const handleFileSelect = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > maxSize * 1024 * 1024) {
        ...
        notify(`File too large. Maximum size is ${maxSize}MB.`, { type: 'error' })
        return
      }
      ...
      try {
        ...
        // Debug logging
        console.log('Upload result:', { imageUrl, type: typeof imageUrl })

        // Validate the returned URL
        if (!imageUrl || typeof imageUrl !== 'string' || !imageUrl.startsWith('/uploads/')) {
          console.error('Invalid URL received:', imageUrl)
          throw new Error(`Invalid URL returned from server: ${JSON.stringify(imageUrl)}`)
        }
        ...
        // Set the URL string directly - no objects
        onChange(imageUrl)
